...
/**
 * Send an interactive media carousel (image cards + URL buttons).
 * Auto-advancing — CTA taps open the browser; no webhook reply.
 */
export async function engineSendInteractiveCarousel(
  ...
  // Inbox preview as text — carousel payload isn't in InteractiveMessagePayload yet.
  const preview =
    args.bodyText.trim() ||
    `Product carousel (${args.cards.length} cards)`
  const { error: msgErr } = await db.from('messages').insert({
    conversation_id: args.conversationId,
    sender_type: 'bot',
    content_type: 'text',
  ...
  if (msgErr) {
    console.error(
      '[flows] carousel sent to Meta but DB insert failed:',
      msgErr.message,
    )
  }
